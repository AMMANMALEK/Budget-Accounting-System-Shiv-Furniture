 ...
 * 
 * This service implements comprehensive access control logic for all business resources.
 * It determines whether users can perform specific operations on different resource types
 * based on their roles and the business rules of the application.
 * 
 * Key Features:
 * - Resource-based access validation (cost centers, budgets, invoices, etc.)
 * - Operation-based permissions (create, read, update, delete, post)
 * - Role-specific access rules (admin vs portal users)
 * - Bulk operation validation with safety limits
 * - Comprehensive permission summaries for UI display
 * - Standardized error responses for access violations
 * 
 * Business Rules:
 * - Admin users: Full access to all operations
 * - Portal users: Read-only access, cannot modify business/financial data
 * - Posted records: Cannot be modified by anyone (enforced elsewhere)
 * 
 * This service is used by access control middleware and can be called directly
 * from controllers to validate operations before execution.
 */
