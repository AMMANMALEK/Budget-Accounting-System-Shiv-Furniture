/**
 * Record Immutability Service
 * 
 * This service enforces the critical business rule that posted financial records
 * cannot be modified or deleted. It provides validation methods to ensure
 * accounting integrity and audit compliance throughout the system.
 * 
 * Key Features:
 * - Record modification validation based on status
 * - Record deletion validation with status checking
 * - Type-specific validation for invoices, purchase bills, production expenses
 * - Bulk operation validation for multiple records
 * - Standardized error responses for immutability violations
 * - Middleware-style validation functions
 * 
 * Business Rules:
 * - Draft records: Can be updated, deleted, or posted
 * - Posted records: Cannot be modified or deleted (immutable)
 * - Unknown status records: Treated as immutable for safety
 * 
 * Record Types Supported:
 * - Invoices
 * - Purchase Bills
 * - Production Expenses
 * - Generic records (extensible pattern)
 * 
 * This service is fundamental to maintaining financial data integrity and
