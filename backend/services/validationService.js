 ...
 * Validation Service
 * 
 * This service provides comprehensive data validation for all business entities
 * in the budget and expense tracking system. It implements validation rules for
 * required fields, data types, formats, business constraints, and relationships.
 * 
 * Key Features:
 * - Field-level validation (required, type, format, length)
 * - Business rule validation (date ranges, positive numbers, email format)
 * - Entity-specific validation (budgets, invoices, cost centers, etc.)
 * - Budget overlap detection to prevent conflicting budget periods
 * - Post operation validation (only draft records can be posted)
 * - Bulk operation validation with safety limits
 * - Standardized error response formatting
 * 
 * Validation Rules:
 * - All monetary amounts must be positive numbers
 * - Date ranges must have start date before end date
 * - Budget periods cannot overlap for the same cost center
 * - Required fields must not be null, undefined, or empty strings
 * - String fields have minimum and maximum length constraints
 * 
 * This service is used throughout the application to ensure data integrity
